...
def valid_word(words)
  return false unless words.length == Set.new(words).length

  sorted_words = words.map(&:chars).map(&:sort)
  sorted_words.length == Set.new(sorted_words).length ? true : false
end

number_of_valid = input.split("\n").reduce(0) do |valid, passphrase|
  words = passphrase.split(" ")
  valid += 1 if valid_word(words)
  valid
end
