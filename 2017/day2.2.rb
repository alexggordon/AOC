...
571\t547\t160\t237\t256\t30\t496\t592\t385\t576\t183\t692\t192\t387\t647\t233".split("\n")


# finds the only two numbers in each row where one evenly divides the other
# * remember, in division, dividend / divisor = quotient
#
def compute_checksum_for_row(row)
  row.each do |possible_divisor|
    possible_divisors_for_val = row.select do |num|
      if num == possible_divisor
        false
      elsif num > possible_divisor/2
        false
      elsif (possible_divisor % num) == 0
        true
      end
    end
    return possible_divisor / possible_divisors_for_val.first if possible_divisors_for_val.length > 0
  end
end

result = input.map{ |row| row.split("\t").map(&:to_i) }
  .inject(0) {|reducer, row| reducer += compute_checksum_for_row(row) }
