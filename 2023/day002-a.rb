...
input.each do |line|
  ...
  header_num = header.split("Game ").last.to_i
  rounds = tail.split(";")
  total += header_num if rounds.all? do |round|
    # puts round
    iter = round.split(", ")
    # puts iter.to_json
    iter.all? do |mtch|    
      ...
      when /red/
        # puts "red"
        # puts mtch.split(" ").first.to_i < 12
        mtch.split(" ").first.to_i <= 12
      when /green/
        mtch.split(" ").first.to_i <= 13
      when /blue/
        mtch.split(" ").first.to_i <= 14
      end
    end
  end
end
