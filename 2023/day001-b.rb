...
input = %w(eighttkbtzjz6nineeight
...
input.each do |line|
  # first_words = letters.map{|x| [x, line.index(x)]}.reject{|x| x[1].nil?}
  first_words = []
  letters.map { |e| first_words.concat(matches(line, e).map{|x| [e, x]}) }
  first_words.reject{|x| x.count == 0}
  # puts first_words.to_json
  first_word = first_words.sort_by{|x| x[1]}.first
  first_num = line.chars.find {|x| (0..9).to_a.map(&:to_s).include?(x)}
  first_idx = line.index(first_num)
  puts first_words.to_json
  # puts first_word.class
  if first_word.blank?
    first = first_num
  elsif first_idx < first_word[1]
    first = first_num
  else
    word = first_word[0].dup
    word.gsub!("one", "1")
    word.gsub!("two", "2")
    word.gsub!("three", "3")
    word.gsub!("four", "4")
  ...
  # last_words = letters.map{|x|  [x, line.index(x)]}.reject{|x| x[1].nil?}
  # last_words = letters.map { |e| matches(line, e) }.flatten
  last_word = first_words.sort_by{|x| x[1]}.last
  last_num = line.reverse.chars.find {|x| (0..9).to_a.map(&:to_s).include?(x)}
  # matches(last_num).max
  last_idx = matches(line, last_num).max

  # puts first_words.to_json
  if last_word.blank?
    last = last_num
  elsif last_idx > last_word[1]
    last = last_num
  else
    word = last_word[0].dup
    word.gsub!("one", "1")
    word.gsub!("two", "2")
    word.gsub!("three", "3")
    word.gsub!("four", "4")
  ...
  puts [first, line, last].join("-")
  count += [first, last].join.to_i

  # puts [line, first, last].join("-")
end
