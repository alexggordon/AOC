...
def read_til_end(line, pos)

  (orig_pos = pos.dup).freeze
  nums = []

  return nums if !@nums.include?(line[pos]) && !@nums.include?(line[pos + 1]) && !@nums.include?(line[pos - 1])

  if @nums.include?(line[pos])
    num = ""
    while @nums.include?(line[pos]) && pos > 0 && @nums.include?(line[pos -1])
      (pos -= 1)
    end
    # binding.pry
    while @nums.include?(line[pos]) && pos <= 139
      num += line[pos]
      pos += 1
    end

    return [num].select(&:present?).map(&:to_i)
  else

    rnum = ""
    lnum = ""
    pos = pos += 1
    while @nums.include?(line[pos])
      rnum += line[pos]
      pos += 1
    end

    pos = orig_pos.dup - 1
    while @nums.include?(line[pos]) && pos >= 0
    ...
    # binding.pry
    return [rnum, lnum].select(&:present?).map(&:to_i)
  end
end
...
input.each_with_index do |line, line_num|
  line.chars.each_with_index do |point, curr_pos|
    ...
    # binding.pry
    # read right
    nums.concat(read_til_end(line, curr_pos))
    # read above (left and right)
    nums.concat(read_til_end(input[line_num - 1], curr_pos))
    # binding.pry
    # read below (left and right?)
    nums.concat(read_til_end(input[line_num + 1], curr_pos))
    # binding.pry
    if nums.count == 2
      @sum += nums.inject(:*) 
