...
instructions = instructions.chars.map{|x| x == "L" ? 0 : 1}

map = map.split("\n").reduce({}) {|m, line|
  m[line.split(" = ").first] = line.split(" = ").last.remove("(").remove(")").split(", ")
  m
}

puts map.keys.select{ _1.last == "A" }.map {
  step = 0
  node = _1
  while node.last != "Z"
    node = map[node][instructions[step % (instructions.size)]]
    step += 1
  end
  step
}.inject(&:lcm)
