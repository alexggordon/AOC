...
def check_point(input, line_num, curr_pos)
  valid = false
  prev_line = line_num == 0 ? nil : input[line_num - 1]
  next_line = line_num == (input.count - 1) ? nil : input[line_num + 1]

  valid = true if @sym.include?(input[line_num][curr_pos])
  valid = true if prev_line.present? && @sym.include?(prev_line[curr_pos])
  valid = true if next_line.present? && @sym.include?(next_line[curr_pos])
  valid
end
...
require 'pry'
@sum = 0
input.each_with_index do |line, line_num|
  @inside_number = false
  @num_so_far = ""
  @number_counts = false
  line.chars.each_with_index do |point, curr_pos|
    if !@nums.include?(point) && !@inside_number
      @number_counts = true if @nums.include?(line[curr_pos + 1]) && (check_point(input, line_num, curr_pos))
      next
    end

    @number_counts = true if check_point(input, line_num, curr_pos) && @inside_number



    if @inside_number && !@nums.include?(point) # num term
      # binding.pry
      if @number_counts
        @sum += @num_so_far.to_i
      else
      ...
      @inside_number = false
      @num_so_far = ""
      @number_counts = false unless @nums.include?(line[curr_pos + 1]) && (check_point(input, line_num, curr_pos))
    elsif @inside_number && @nums.include?(point) # num mid
      @num_so_far += point

      #check if EOL
      @sum += @num_so_far.to_i if @number_counts if curr_pos == (line.length - 1)

    elsif !@inside_number && @nums.include?(point) # num begin
      @number_counts = true if check_point(input, line_num, curr_pos)
      @inside_number = true
      @num_so_far += point

      #check if EOL
      @sum += @num_so_far.to_i if @number_counts && curr_pos == (line.length - 1)
    end
  end
end
