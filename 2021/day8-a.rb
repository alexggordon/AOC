...
input.each do |c|
  ...
  codes.select{|x| known_examples.keys.include?(x.length) }.each do |code|
    solved_num[known_examples[code.length]].push(code.chars)
  end
  ...
  # solves b
  codes.select{|x| x.length == 5}.select do |code|
    four = codes.find{|x| x.length == 4}
    one = codes.find{|x| x.length == 1}

    return true unless one.chars.all?{ |m| code.chars.include?(m)}
