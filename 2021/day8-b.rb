...
def check(hsh)
  ...
  found = []
  hsh.keys.each do |k|
    found.push(hsh[k].first) if hsh[k].length == 1
  end
...
input.each do |c|
  ...
  codes.select{|x| known_examples.keys.include?(x.length) }.each do |code|
    solved_num[known_examples[code.length]].concat(code.chars)
  end

  # hints 
  if solved_num[4].present? && solved_num[8].present?
    solved_letters["a"] = solved_num[7] - solved_num[1]
  end

  # solves a
  if solved_num[7].present? && solved_num[1].present?
    solved_letters["a"] = solved_num[7] - solved_num[1]
    solved_letters["b"] = solved_num[7] - solved_letters["a"]
    solved_letters["c"] = solved_num[7] - solved_letters["a"]
  else 
    raise 
  end

  # solves b
  codes.select{|x| x.length == 5}.each do |code|
    four = codes.find{|x| x.length == 4}.chars
    one = codes.find{|x| x.length == 2}.chars
    diff = four - one
    # puts "___"
    # puts four.to_s
    # puts one.to_s
    # puts diff.to_s
    solved_letters["f"] = diff
    solved_letters["g"] = diff
    # 4       on  on          on  on      00110011  0x33  01100110  0x66
    # 1       on  on                      00110000  0x30  00000110  0x06 
    #                         on  on

    # 2   on  on      on  on      on      01101101  0x6D  01011011  0x5B
    # 3   on  on  on  on          on      01111001  0x79  01001111  0x4F
    # 5   on      on  on      on  on      01011011  0x5B  01101101  0x6D
    next if one.all?{ |m| code.chars.include?(m) }
    next if diff.all?{ |m| code.chars.include?(m) }
    # 4       on  on          on  on      00110011  0x33  01100110  0x66
    # 2   on  on      on  on      on      01101101  0x6D  01011011  0x5B
    # 1       on  on                      00110000  0x30  00000110  0x06 
    (four + one + code.chars).uniq do |let|
      if four.include?(let) && one.include?(let) && code.chars.include?(let)
        solved_letters["b"] = [let]
      end
    end
  end


  solved_letters = check(solved_letters)

  # puts solved_letters


  # solve d
  if solved_letters["a"].length == 1 && solved_letters["b"].length == 1 && solved_letters["c"].length == 1
    # puts codes.select{|x| x.length == 5}
    codes.select{|x| x.length == 5}.each do |code|
      required = solved_letters["a"] + solved_letters["b"] + solved_letters["c"]
      # puts required.to_s
      next unless required.all?{|e| code.chars.include?(e) }
      filterd = code.chars - required
      filterd = filterd - solved_letters["g"]
      raise unless filterd.length == 1
      solved_letters["d"] = filterd
    end
  end
  ...
  # solve e
  if solved_letters["a"].length == 1 && solved_letters["b"].length == 1 && solved_letters["c"].length == 1 && solved_letters["d"].length == 1
    required = solved_letters["a"] + solved_letters["b"] + solved_letters["c"] + solved_letters["d"]
    four = codes.find{|x| x.length == 4}.chars
    eight = codes.find{|x| x.length == 7}.chars
    x = eight - (four + required).uniq
  ...
  solved_letters = check(solved_letters)
  # solve f and g
  if solved_letters["a"].length == 1 && solved_letters["b"].length == 1 && solved_letters["c"].length == 1 && solved_letters["d"].length == 1 && solved_letters["e"].length == 1
    # 0   on  on  on  on  on  on          01111110  0x7E  00111111  0x3F
    # 6   on      on  on  on  on  on      01011111  0x5F  01111101  0x7D
    # 9   on  on  on  on      on  on      01111011  0x7B  01101111  0x6F
    codes.select{|x| x.length == 6}.each do |code|
      required = solved_letters["a"] + solved_letters["b"] + solved_letters["c"] + solved_letters["d"] + solved_letters["e"]
      next unless (code.chars - required).count == 1
      old = solved_letters["f"]
      solved_letters["f"] = code.chars - required
      solved_letters["g"] = old - solved_letters["f"]
    end
  end
  require 'pry'
  binding.pry unless solved_letters.values.all?{|x| x.length == 1}
  solved_letters.keys.each do |k|
    solved_letters[k] = solved_letters[k].first
  end
  real_answer = {
    "abcdef"  => 0,
    "bc"      => 1,
  ...
  ans = output.map do |code|
    ans = code.chars.map {|e| solved_letters.key(e)}.sort.join("")
    real_answer[ans]
  end
  @real_ansers.push(ans.join.to_i)
end


puts @real_ansers.sum
...
#     a   b   c   d   e   f   g
# 0   🩸  on  on  on  on  on          01111110  0x7E  00111111  0x3F
# 1       on  on                      00110000  0x30  00000110  0x06
# 2   🩸  on      on  on      on      01101101  0x6D  01011011  0x5B
# 3   🩸  on  on  on          on      01111001  0x79  01001111  0x4F
# 4       on  on          on  on      00110011  0x33  01100110  0x66
# 5   🩸      on  on      on  on      01011011  0x5B  01101101  0x6D
# 6   🩸      on  on  on  on  on      01011111  0x5F  01111101  0x7D
# 7   🩸  on  on                      01110000  0x70  00000111  0x07
# 8   🩸  on  on  on  on  on  on      01111111  0x7F  01111111  0x7F
# 9   🩸  on  on  on      on  on      01111011  0x7B  01101111  0x6F



# puts input.join(" ").split(" ").select{|x| examples.keys.include?(x.length) }.count
